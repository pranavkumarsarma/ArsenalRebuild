...
class TransfermarktBaseModel(BaseModel):
    ...
    def parse_str_to_int(cls, v) -> Optional[int]:
        if isinstance(v, int):
            return v
        if not v or not isinstance(v, str) or not any(char.isdigit() for char in v):
            return None
        ...
        if "<" in v:
            ...
            value_str = v.lower().replace("€", "").replace("£", "").replace("+", "").replace("'", "").strip()

        try:
            if "k" in value_str:
                return int(float(re.sub(r"[^\d.]", "", value_str.split("k")[0])) * 1_000)
            elif "m" in value_str:
                return int(float(re.sub(r"[^\d.]", "", value_str.split("m")[0])) * 1_000_000)
            elif "bn" in value_str or "b" in value_str:
                return int(float(re.sub(r"[^\d.]", "", re.split(r"b", value_str)[0])) * 1_000_000_000)
            else:
